...
function prosesOnline(id) {
    ...
    document.getElementById('manual-telp').value = order.telp; // Masukkan telp ke input manual
...
// UPDATE: Fungsi Tambah Pesanan sekarang mengambil input No. Telepon
function tambahPesanan() {
    const n = document.getElementById('nama').value;
    const tlpManual = document.getElementById('manual-telp').value; // AMBIL DATA TELP
    ...
    dataLaundry.push({ 
        id: Date.now(), 
        nama: n, 
        telp: tlpManual || tempTelp || "-", // PRIORITASKAN INPUT MANUAL
    ...
    document.getElementById('manual-telp').value = ""; // BERSIHKAN TELP
    ...
    saveData();
    alert("Berhasil disimpan ke antrian!");
}

function selesaikan(id) {
    dataLaundry = dataLaundry.map(i => i.id === id ? {...i, status: 'Selesai', waktuSelesai: Date.now()} : i);
    saveData();
}

function hapusRiwayat(id) {
    if(confirm("Hapus data ini?")) {
        dataLaundry = dataLaundry.filter(i => i.id !== id);
        saveData();
    }
}
...
function hapusSemuaRiwayat() {
    if(confirm("Hapus semua riwayat yang sudah SELESAI?")) {
        dataLaundry = dataLaundry.filter(i => i.status === 'Proses');
        saveData();
    }
}
...
function renderAll() {
    ...
    if(tbO) {
        tbO.innerHTML = "";
        [...dataOnline].sort((a,b) => b.id - a.id).forEach(item => {
            tbO.innerHTML += `<tr><td><strong>${item.nama}</strong></td><td>Jarak: ${item.jarak} km<br>Ongkir: Rp ${item.ongkir.toLocaleString()}</td><td><button onclick="prosesOnline(${item.id})" style="background:var(--sky); border:none; padding:5px 10px; border-radius:8px; cursor:pointer;">Proses</button></td></tr>`;
        });
    }
    ...
    if(tbA && tbH) {
        ...
        dataLaundry.filter(i => i.status === 'Proses').sort((a,b) => b.id - a.id).forEach(item => {
            tbA.innerHTML += `<tr><td>${item.nama}</td><td>${item.detail}</td><td>Rp ${item.total.toLocaleString()}</td><td><span style="color:orange;">● Proses</span></td><td><button onclick="selesaikan(${item.id})" style="color:green; border:none; background:none; cursor:pointer; font-weight:bold;">Selesai</button></td></tr>`;
        });

        dataLaundry.filter(i => i.status === 'Selesai').sort((a,b) => b.waktuSelesai - a.waktuSelesai).forEach(item => {
            tbH.innerHTML += `<tr><td>${item.nama}</td><td>Rp ${item.total.toLocaleString()}</td><td style="display:flex; gap:5px;">
    ...
    if (detailBox && modal) {
        detailBox.innerHTML = `
            <div style="background: #f9f9f9; padding: 15px; border-radius: 10px; border-left: 5px solid var(--deep-blue);">
                ...
                <p style="font-size: 11px; margin-top: 10px; opacity: 0.6;">Selesai pada: ${new Date(item.waktuSelesai).toLocaleString('id-ID')}</p>
            </div>`;
...
function saveData() { localStorage.setItem('laundryData', JSON.stringify(dataLaundry)); renderAll(); }

// Jalankan render awal saat script load
renderAll();